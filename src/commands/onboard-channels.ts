...
async function noteChannelPrimer(
  ...
  await prompter.note(
    [
      "私信安全: 默认配对；未知私信将获得配对码。",
      `批准命令: ${formatCliCommand("openclaw pairing approve <channel> <code>")}`,
      '公开私信需要 dmPolicy="open" + allowFrom=["*"].',
      '多用户私信: 设置 session.dmScope="per-channel-peer" (或多账户渠道的 "per-account-channel-peer") 以隔离会话。',
...
export async function setupChannels(
  ...
  const ensureBundledPluginEnabled = async (channel: ChannelChoice): Promise<boolean> => {
    ...
    if (!result.enabled) {
      await prompter.note(
        `Cannot enable ${channel}: ${result.reason ?? "插件已禁用"}.`,
        "Channel setup",
      );
      return false;
    }
    ...
    if (!getChannelPlugin(channel)) {
      await prompter.note(`${channel} 插件不可用。`, "Channel setup");
      return false;
    }
  ...
  const configureChannel = async (channel: ChannelChoice) => {
    const adapter = getChannelOnboardingAdapter(channel);
    if (!adapter) {
      await prompter.note(`${channel} 暂不支持初始化向导。`, "Channel setup");
      return;
    }
  ...
  const handleConfiguredChannel = async (channel: ChannelChoice, label: string) => {
    ...
    if (action === "delete" && !supportsDelete) {
      await prompter.note(`${label} 不支持删除配置条目。`, "Remove channel");
      return;
    }
    ...
    const choice = (await prompter.select({
      ...
      options: [
        ...buildSelectionOptions(entries),
        {
          value: "__skip__",
          label: "暂时跳过",
          hint: `你可以稍后通过 \`${formatCliCommand("openclaw channels add")}\` 添加渠道`,
        },
      ],
