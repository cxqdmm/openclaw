...
export async function applyAuthChoiceApiProviders(
  ...
  if (authChoice === "cloudflare-ai-gateway-api-key") {
    ...
    const ensureAccountGateway = async () => {
      if (!accountId) {
        const value = await params.prompter.text({
          message: "输入 Cloudflare 账户 ID",
          validate: (val) => (String(val).trim() ? undefined : "账户 ID 必填"),
        });
        accountId = String(value).trim();
      }
      if (!gatewayId) {
        const value = await params.prompter.text({
          message: "输入 Cloudflare AI 网关 ID",
          validate: (val) => (String(val).trim() ? undefined : "网关 ID 必填"),
        });
    ...
    if (!hasCredential && envKey) {
      const useExisting = await params.prompter.confirm({
        message: `使用现有的 CLOUDFLARE_AI_GATEWAY_API_KEY (${envKey.source}, ${formatApiKeyPreview(envKey.apiKey)})?`,
        initialValue: true,
      });
      if (useExisting) {
    ...
    if (!hasCredential) {
      await ensureAccountGateway();
      const key = await params.prompter.text({
        message: "输入 Cloudflare AI 网关 API 密钥",
        validate: validateApiKeyInput,
      });
      await setCloudflareAiGatewayConfig(
        accountId,
  ...
  if (authChoice === "together-api-key") {
    ...
    if (!hasCredential) {
      await params.prompter.note(
        [
          "Together AI 提供对领先开源模型（包括 Llama, DeepSeek, Qwen 等）的访问。",
    ...
    if (envKey) {
      const useExisting = await params.prompter.confirm({
        message: `使用现有的 TOGETHER_API_KEY (${envKey.source}, ${formatApiKeyPreview(envKey.apiKey)})?`,
        initialValue: true,
      });
      if (useExisting) {
  ...
  if (authChoice === "qianfan-api-key") {
    ...
    if (envKey) {
      const useExisting = await params.prompter.confirm({
        message: `使用现有的 QIANFAN_API_KEY (${envKey.source}, ${formatApiKeyPreview(envKey.apiKey)})?`,
        initialValue: true,
      });
      if (useExisting) {
