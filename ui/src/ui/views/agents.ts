...
const TOOL_SECTIONS = [
  {
    ...
    tools: [
      ...
      { id: "apply_patch", label: "apply_patch", description: "修补文件（OpenAI）" },
    ],
  },
  {
    id: "runtime",
...
function buildAgentContext(
  ...
  return {
    ...
    identityName,
    identityEmoji,
    skillsLabel: skillFilter ? `${skillCount} 已选择` : "所有技能",
...
function resolveModelLabel(model?: unknown): string {
  ...
  if (typeof model === "object" && model) {
    ...
    if (primary) {
      ...
      return fallbackCount > 0 ? `${primary} (+${fallbackCount} 后备)` : primary;
    }
  }
  return "-";
}
...
export function renderAgents(props: AgentsProps) {
  ...
  const 已选择Id = props.已选择AgentId ?? defaultId ?? agents[0]?.id ?? null;
  const 已选择Agent = 已选择Id
    ? (agents.find((agent) => agent.id === 已选择Id) ?? null)
    : null;
      ...
      <section class="card agents-sidebar">
        <div class="row" style="justify-content: space-between;">
          <div>
            ...
            <div class="card-sub">${agents.length} 个已配置。</div>
          </div>
            ...
            agents.length === 0
              ...
              : agents.map((agent) => {
                  ...
                  return html`
                    <button
                      type="button"
                      class="agent-row ${已选择Id === agent.id ? "active" : ""}"
          ...
          !已选择Agent
            ...
            : html`
              ${renderAgentHeader(
                已选择Agent,
                defaultId,
                props.agentIdentityById[已选择Agent.id] ?? null,
              )}
              ...
              ${
                props.activePanel === "overview"
                  ? renderAgentOverview({
                      ...
                      agentIdentity: props.agentIdentityById[已选择Agent.id] ?? null,
    ...
    <section class="card">
      ...
      <div class="agents-overview-grid" style="margin-top: 16px;">
        ...
        <div class="agent-kv">
          ...
          <div>${skillFilter ? `${skillCount} 已选择` : "所有技能"}</div>
        </div>
      </div>
      ...
      <section class="card">
        <div class="row" style="justify-content: space-between;">
          <div>
            ...
            <div class="card-sub">网关范围的渠道状态快照。</div>
          </div>
        ...
        ${
          !params.snapshot
            ? html`
                <div class="callout info" style="margin-top: 12px">加载渠道以查看实时状态。</div>
              `
            : nothing
        }
        ${
          entries.length === 0
            ? html`
                <div class="muted" style="margin-top: 16px">未找到渠道。</div>
              `
            : html`
...
function renderAgentCron(params: {
  ...
  return html`
      ...
      <section class="card">
        ...
        <div class="stat-grid" style="margin-top: 16px;">
          ...
          <div class="stat">
            ...
            <div class="stat-value">${params.status?.jobs ?? "不可用"}</div>
          </div>
          <div class="stat">
    ...
    <section class="card">
      ...
      <div class="card-sub">针对此代理的计划任务。</div>
      ${
        jobs.length === 0
          ? html`
              <div class="muted" style="margin-top: 16px">暂无分配任务。</div>
            `
          : html`
              <div class="list" style="margin-top: 16px;">
                ${jobs.map(
    ...
    <section class="card">
      <div class="row" style="justify-content: space-between;">
        <div>
          ...
          <div class="card-sub">引导角色、身份和工具指导。</div>
        </div>
        <button
          class="btn btn--sm"
    ...
    <section class="card">
      ...
      ${
        hasGlobalAllow
          ? html`
              <div class="callout info" style="margin-top: 12px">
                已设置全局 tools.allow。代理覆盖无法启用全局阻止的工具。
              </div>
            `
          : nothing
      }
  ...
  return html`
    <section class="card">
      ...
      <div class="filters" style="margin-top: 14px;">
        ...
        <div class="muted">${filtered.length} 已显示</div>
      </div>
    ...
    <div class="list-item agent-skill-row">
      <div class="list-main">
        ...
        <div class="chip-row" style="margin-top: 6px;">
          ...
          <span class="chip ${skill.符合条件 ? "chip-ok" : "chip-warn"}">
            ${skill.符合条件 ? "符合条件" : "已阻止"}
          </span>
          ${
            skill.disabled
