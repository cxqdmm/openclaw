...
export type NodesProps = {
  ...
  onDevice批准: (requestId: string) => void;
  onDevice拒绝: (requestId: string) => void;
  onDevice轮换: (deviceId: string, role: string, scopes?: string[]) => void;
  onDevice撤销: (deviceId: string, role: string) => void;
  onLoadConfig: () => void;
  ...
  onExecApprovals移除: (path: Array<string | number>) => void;
    ...
    <div class="list-item">
      ...
      <div class="list-meta">
        <div class="row" style="justify-content: flex-end; gap: 8px; flex-wrap: wrap;">
          <button class="btn btn--sm primary" @click=${() => props.onDevice批准(req.requestId)}>
            批准
          </button>
          <button class="btn btn--sm" @click=${() => props.onDevice拒绝(req.requestId)}>
            拒绝
          </button>
        </div>
      </div>
  ...
  return html`
    <div class="row" style="justify-content: space-between; gap: 8px;">
      ...
      <div class="row" style="justify-content: flex-end; gap: 6px; flex-wrap: wrap;">
        <button
          class="btn btn--sm"
          @click=${() => props.onDevice轮换(deviceId, token.role, token.scopes)}
        >
          轮换
        </button>
        ${
          token.revokedAtMs
            ...
            : html`
              <button
                ...
                @click=${() => props.onDevice撤销(deviceId, token.role)}
              >
                撤销
              </button>
            `
...
type ExecApprovalsState = {
  ...
  defaults: ExecApprovalsResolved默认值;
  ...
  on移除: (path: Array<string | number>) => void;
  onLoad: () => void;
  onSave: () => void;
};
...
function resolveExecApprovals默认值(
  form: ExecApprovalsFile | null,
): ExecApprovalsResolved默认值 {
...
function resolveExecApprovalsState(props: NodesProps): ExecApprovalsState {
  ...
  const defaults = resolveExecApprovals默认值(form);
    ...
    <div class="list" style="margin-top: 12px;">
      ...
      ${
        state.target === "node" && !hasNodes
          ? html`
              <div class="muted">尚无节点提供执行审批功能。</div>
            `
          : nothing
      }
    </div>
...
function renderExecApprovalsPolicy(state: ExecApprovalsState) {
  const is默认值 = state.selectedScope === EXEC_APPROVALS_DEFAULT_SCOPE;
  const defaults = state.defaults;
  const agent = state.selectedAgent ?? {};
  const basePath = is默认值 ? ["defaults"] : ["agents", state.selectedScope];
  const agentSecurity = typeof agent.security === "string" ? agent.security : undefined;
  const agentAsk = typeof agent.ask === "string" ? agent.ask : undefined;
  const agentAskFallback = typeof agent.askFallback === "string" ? agent.askFallback : undefined;
  const securityValue = is默认值 ? defaults.security : (agentSecurity ?? "__default__");
  const askValue = is默认值 ? defaults.ask : (agentAsk ?? "__default__");
  const askFallbackValue = is默认值 ? defaults.askFallback : (agentAskFallback ?? "__default__");
  const autoOverride =
    ...
    <div class="list" style="margin-top: 16px;">
      <div class="list-item">
        <div class="list-main">
          ...
          <div class="list-sub">
            ${is默认值 ? "Default security mode." : `Default: ${defaults.security}.`}
          </div>
        </div>
        <div class="list-meta">
          <label class="field">
            ...
            <select
              ...
              @change=${(event: Event) => {
                const target = event.target as HTMLSelectElement;
                const value = target.value;
                if (!is默认值 && value === "__default__") {
                  state.on移除([...basePath, "security"]);
                } else {
      ...
      <div class="list-item">
        <div class="list-main">
          ...
          <div class="list-sub">
            ${is默认值 ? "Default prompt policy." : `Default: ${defaults.ask}.`}
          </div>
        </div>
        <div class="list-meta">
          <label class="field">
          ...
          <label class="field">
            ...
            <select
              ...
              @change=${(event: Event) => {
                ...
                const value = target.value;
                if (!is默认值 && value === "__default__") {
                  state.on移除([...basePath, "askFallback"]);
                } else {
          ...
          ${
            !is默认值 && !autoIsDefault
              ? html`<button
                class="btn btn--sm"
                ?disabled=${state.disabled}
                @click=${() => state.on移除([...basePath, "autoAllowSkills"])}
              >
                使用默认
              </button>`
              : nothing
        ...
        <button
          ...
          @click=${() => {
            if (state.allowlist.length <= 1) {
              state.on移除(["agents", state.selectedScope, "allowlist"]);
              return;
            }
            state.on移除(["agents", state.selectedScope, "allowlist", index]);
          }}
        >
          移除
        </button>
