    ...
    return html`
      <div class="card usage-mosaic">
        <div class="usage-mosaic-header">
          <div>
            <div class="usage-mosaic-title">按时间活动</div>
      ...
      <div class="usage-mosaic-header">
        <div>
          ...
          <div class="usage-mosaic-sub">
            根据会话时间跨度估算（首次/末次活动）。时区: ${timeZone === "utc" ? "UTC" : "本地"}。
          </div>
        </div>
        ...
        selectedDays.length > 0
          ? html`
            <div class="filter-chip">
              <span class="filter-chip-label">天数: ${daysLabel}</span>
              <button class="filter-chip-remove" @click=${onClearDays} title="移除筛选器">×</button>
            </div>
          `
          : nothing
      }
        ...
        selectedHours.length > 0
          ? html`
            <div class="filter-chip">
              ...
              <button class="filter-chip-remove" @click=${onClearHours} title="移除筛选器">×</button>
            </div>
          `
          : nothing
      }
        ...
        selectedSessions.length > 0
          ? html`
            <div class="filter-chip" title="${sessionsFullName}">
              ...
              <button class="filter-chip-remove" @click=${onClearSessions} title="移除筛选器">×</button>
            </div>
          `
          : nothing
      }
  ...
  if (!daily.length) {
    return html`
      <div class="daily-chart-compact">
        <div class="sessions-panel-title">每日使用量</div>
  ...
  return html`
    <div class="daily-chart-compact">
      <div class="daily-chart-header">
        ...
        <div class="card-title">每日${isTokenMode ? "令牌" : "成本"}使用量</div>
      </div>
...
function renderCostBreakdownCompact(totals: UsageTotals, mode: "tokens" | "cost") {
  ...
  return html`
    <div class="cost-breakdown cost-breakdown-compact">
      <div class="cost-breakdown-header">${isTokenMode ? "令牌" : "成本"}按类型</div>
...
function renderUsageInsights(
  ...
  const throughputHint = "吞吐量显示活动时间内每分钟的令牌数。越高越好。";
  const tokensHint = "此范围内每条消息的平均令牌数。";
    ...
    <section class="card" style="margin-top: 16px;">
      ...
      <div class="usage-summary-grid">
        ...
        <div class="usage-summary-card">
          <div class="usage-summary-title">
            Tool Calls
            <span class="usage-summary-hint" title="跨会话的工具调用总数。">?</span>
          </div>
          <div class="usage-summary-value">${aggregates.tools.totalCalls}</div>
          <div class="usage-summary-sub">${aggregates.tools.uniqueTools} 个工具使用</div>
        </div>
        <div class="usage-summary-card">
          <div class="usage-summary-title">
            Errors
      ...
      <div class="usage-insights-grid">
        ...
        ${renderInsightList("热门代理", topAgents, "无代理数据")}
        ${renderInsightList("热门渠道", topChannels, "无渠道数据")}
        ${renderPeakErrorList("错误峰值日期", errorDays, "无错误数据")}
        ${renderPeakErrorList("错误峰值小时", errorHours, "无错误数据")}
      </div>
    </section>
  `;
}
  ...
  return html`
    ...
    <div class="session-summary-grid">
      ...
      <div class="session-summary-card">
        ...
        <div class="session-summary-meta">${usage.messageCounts?.toolResults ?? 0} 个工具结果</div>
      </div>
      <div class="session-summary-card">
    ...
    <div class="usage-insights-grid" style="margin-top: 12px;">
      ...
      ${renderInsightList("模型组合", modelItems, "无模型数据")}
    </div>
  `;
}
  ...
  return html`
    <div class="session-timeseries-compact">
      <div class="timeseries-header-row">
        <div class="card-title" style="font-size: 13px;">随时间变化的用量</div>
        ...
        breakdownByType
          ? html`
              <div style="margin-top: 8px;">
                <div class="card-title" style="font-size: 12px; margin-bottom: 6px;">按类型的令牌</div>
                ...
                <div class="cost-breakdown-legend">
                  ...
                  <div class="legend-item" title="用户 + 工具输入令牌">
  ...
  return html`
    <div class="context-details-panel">
      <div class="context-breakdown-header">
        <div class="card-title" style="font-size: 13px;">系统提示词分解</div>
        ${
          hasMore
      ...
      <p class="context-weight-desc">${contextPct || "每条消息的基础上下文"}</p>
      ...
      <div class="context-total">总计: ~${formatTokens(totalContextTokens)}</div>
...
function renderSessionLogsCompact(
  ...
  if (!logs || logs.length === 0) {
    return html`
      <div class="session-logs-compact">
        <div class="session-logs-header">Conversation</div>
        <div class="muted" style="padding: 20px; text-align: center">无消息</div>
      </div>
    `;
  }
  ...
  return html`
    <div class="session-logs-compact">
      <div class="session-logs-header">
        <span>对话 <span style="font-weight: normal; color: var(--text-muted);">(${displayedCount} messages)</span></span>
      ...
      <div class="session-logs-list">
        ...
        ${
          filteredEntries.length === 0
            ? html`
                <div class="muted" style="padding: 12px">没有消息匹配筛选条件。</div>
              `
            : nothing
        }
      </div>
